...
export class CreateSquadConfigComponent implements OnInit {
  ...
  onSubmitForm(event: SubmitEvent): void {
    ...
    let squadConfigCreated = Object.assign(
      {},
      this.squadConfigCreated,
      this.createSquadConfigForm.value,
    );
    const playerTypeSelected = this.playerTypes.find(
      (pt) => pt.id === squadConfigCreated.playerTypeId,
    );
  ...
  private checkDuplicatedSquadConfig(playerTypeId: string): boolean {
    const exist = this.squadConfigsCreated.find(
      (sc) => sc.playerTypeId === playerTypeId,
    );
    return !!exist;
  }
  ...
  private initValidationMessages(): void {
    this.validationMessages = {
      ...
      quantity: {
        required:
          'Defina a quantidade máxima de jogadores para o tipo selecionado!',
      },
    };
  }
