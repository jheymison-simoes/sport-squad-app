...
export class SquadService extends BaseService {
  ...
  create(squad: CreateSquadDto): Observable<SquadDto> {
    ...
    return this.httpClient
      .post<BaseResponse<SquadDto>>(uri, squad)
      .pipe(map(this.validationResult));
  }
  ...
  delete(squadId: string): Observable<SquadDto> {
    ...
    return this.httpClient
      .delete<BaseResponse<SquadDto>>(uri)
      .pipe(map(this.validationResult));
  }
  ...
  getSquadTextShared(squadId: string): Observable<string> {
    ...
    return this.httpClient
      .get<BaseResponse<string>>(uri)
      .pipe(map(this.validationResult));
  }

  assembleTeams(
    assembleTeamsDto: AssembleTeamsDto,
  ): Observable<Array<AssembledTeamsDto>> {
    const uri = `${this.baseUri}/assembleteams`;
    return this.httpClient
      .post<BaseResponse<Array<AssembledTeamsDto>>>(uri, assembleTeamsDto)
      .pipe(map(this.validationResult));
  }

  getTextSharingAssembledTeams(
    squadId: string,
    assembledTeams: Array<AssembledTeamsDto>,
  ): Observable<string> {
    const uri = `${this.baseUri}/sharedtextassembledteams/${squadId}`;
    return this.httpClient
      .post<BaseResponse<string>>(uri, assembledTeams)
      .pipe(map(this.validationResult));
  }
}
