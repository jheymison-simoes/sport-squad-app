...
export class SquadService extends BaseService {
    ...
    getAllByUserId(userId: string): Observable<SquadDto[]> {
        ...
        return this.httpClient.get<BaseResponse<SquadDto[]>>(uri).pipe(map(this.validationResult));
    }

    create(squad: CreateSquadDto): Observable<SquadDto> {
        const uri = `${this.baseUri}`;
        return this.httpClient.post<BaseResponse<SquadDto>>(uri, squad).pipe(map(this.validationResult));
    }

    delete(squadId: string): Observable<SquadDto> {
        const uri = `${this.baseUri}/${squadId}`;
        return this.httpClient.delete<BaseResponse<SquadDto>>(uri).pipe(map(this.validationResult));
    }

    getSquadTextShared(squadId: string): Observable<string> {
        const uri = `${this.baseUri}/gettextsquadshare/${squadId}`;
        return this.httpClient.get<BaseResponse<string>>(uri).pipe(map(this.validationResult));
    }

    assembleTeams(assembleTeamsDto: AssembleTeamsDto): Observable<Array<AssembledTeamsDto>> {
        const uri = `${this.baseUri}/assembleteams`;
        return this.httpClient.post<BaseResponse<Array<AssembledTeamsDto>>>(uri, assembleTeamsDto).pipe(map(this.validationResult));
    }
    ...
    getTextSharingAssembledTeams(squadId: string, assembledTeams: Array<AssembledTeamsDto>): Observable<string> {
        ...
        return this.httpClient.post<BaseResponse<string>>(uri, assembledTeams).pipe(map(this.validationResult));
    }
}
