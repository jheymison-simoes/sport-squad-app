...
export class CreateSquadComponent implements OnInit {
  ...
  onSubmitForm(event: SubmitEvent): void {
    ...
    const squadCreatedData = Object.assign(
      {},
      this.squadCreated,
      this.createSquadForm.value,
    ) as CreateSquadForm;
  ...
  private generateCreateSquadDto(
    squadCreatedData: CreateSquadForm,
  ): CreateSquadDto {
    return {
  ...
  get formIsValid(): boolean {
    if (this.squadConfigsCreated.length === 0) {
      this.notifyService.warning(
        'Não foi adicionada nenhuma configuração do squad!',
      );
      return false;
    }
