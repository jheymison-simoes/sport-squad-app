...
export class PlayerTypeService extends BaseService {
    ...
    getAll(): Observable<PlayerTypeDto[]> {
        return this.httpClient.get<BaseResponse<PlayerTypeDto[]>>(this.baseUri).pipe(map(this.validationResult));
    }

    getAllBySquadId(squadId: string): Observable<PlayerTypeDto[]> {
        const uri = `${this.baseUri}/${squadId}`;
        return this.httpClient.get<BaseResponse<PlayerTypeDto[]>>(uri).pipe(map(this.validationResult));
    }
}
