...
export class AuthenticationService extends BaseService {
  ...
  loginWithGoogle(user: SocialUser): Observable<UserSession> {
    ...
    return this.httpClient
      .post<BaseResponse<UserSession>>(uri, user)
  ...
  get getUserLogged(): UserSession {
    ...
    const accessTokenDecode = JSON.parse(
      Buffer.from(accessToken, 'base64').toString('binary'),
    );
  ...
  get userIsLogged(): boolean {
    ...
    this.userSession = Object.assign(
      {},
      this.userSession,
      accessTokenDecode,
    ) as UserSession;
  ...
  setUserInLocalStorage(userSession: UserSession): void {
    let userSessionJson = Buffer.from(JSON.stringify(userSession)).toString(
      'base64',
    );
