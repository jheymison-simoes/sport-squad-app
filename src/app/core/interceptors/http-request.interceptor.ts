...
export class HttpRequestInterceptor implements HttpInterceptor {
  ...
  errorHandler(error: HttpErrorResponse): string {
    ...
    const messageError = responseError
      ? responseError.errorMessages?.join(', ')
      : 'Houve um erro interno. Tente mais tarde!';

    switch (error.status) {
      case 401:
        this.notifyService.error(
          'Parece que você não possui permissão para esta ação!',
        );
        break;
      case 400:
      ...
      case 503:
        this.notifyService.error(
          'Pedimos desculpa, ocorreu um erro na solicitação ao servidor!',
        );
        break;
    }
